...
exports.get =async (req, res) => {
    ...
    req.session.message="";
    await requestModel.find({kind:'درخواست پشتیبانی اداری'}).then(async function (res2) {
    ...
    if(eshterakstatus.length>0)
        ...
        res2[i].eshterakstatus="0";
            ...
            await factorModel.findOne({requestid:res2[i]._id},function (err,factorresponse) {
        ...
        res.render('./requests/request_list', {
            ...
            activePage: {isAuthenticated: req.isAuthenticated(), sherkat_list: true, title: 'لیست درخواست های پشتیبانی اداری '}
        });
    });
};
